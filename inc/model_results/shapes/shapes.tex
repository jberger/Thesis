...
\begin{equation}
  \xi ( 0 ) = \sqrt{ \frac{ \sigma_{z}(0) }{ \sigma_{\smallT}(0) } } = \frac{ w_z(0) }{ w(0) } = \frac{ v_{\smallzero} \tau(0) }{ w(0) } \,\text{,}
\end{equation}
...
\ref{fig:compare_shape} illustrates the typical dynamics of initially spherical ($ \xi(0) = 0 $), oblate or `disk-like' ($ \xi(0) < 1 $), and prolate or `cigar-like' ($ \xi(0) > 1 $) pulses.
%TODO rewrite using w?
This simulation uses the idealized pulse conditions described above, where the initial bunch dimensions are given by the ellipticities $ \xi(0) = \{ 1/3 , 1 , 3 \} $ and in each case the product of the initial HW1/eM bunch widths $w^2 w_z = $ 1mm$^3$.
Pulses used in \ref{fig:compare_shape}\subref{fig:compare_shape_N1} have just one electron per pulse (i.e. $ N = 10^{0} $), while \ref{fig:compare_shape}\subref{fig:compare_shape_N1e6} have a much higher $ N = 10^{6} $ electrons in the bunch.
Since the initial pulse volume is consistent between all runs, initial peak pulse charge density is six orders of magnitude greater in \ref{fig:compare_shape}\subref{fig:compare_shape_N1e6} than in \ref{fig:compare_shape}\subref{fig:compare_shape_N1}.
...
\begin{figure}
  ...
  \caption[AG simulation of free-space pulse expansion]{
    ...
    The pulses shown in \subref{fig:compare_shape_N1} all have total charge $N=1$, while in \subref{fig:compare_shape_N1e6} the pulses have $N=10^6$.
  }
  \label{fig:compare_shape}
\end{figure}

The bunch propagation dynamics for negligible pulse charge density case shown in \ref{fig:compare_shape}\subref{fig:compare_shape_N1} display the trends expected from the initial photoemission conditions.
...
% the following sentance is no longer true, still perhaps should include something like it
