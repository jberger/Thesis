...
In the transverse ($ \smallT $) direction, a magnetic lens may be modeled using one linear force contribution.
Unfortunately, it is not practical to write an analytic form of the magnitude of this term; the field distribution is surprisingly complicated to describe \cite{montgomery_some_1961} and the ``lensing'' action is in fact a compound force (first rotating the beam, then lensing).
Further discussion of the mathematics of magnetic lenses can be found in Section \ref{sec:mag_lens}.
We have found that in practice, characterizing a specific magnetic lens ($M_{\smallT}^{mag}(I)$ where $I$ is the current in the magnetic lens) is much easier than attempting to find a generic functional form.
...
\begin{equation} \label{eq:RF_Force_z}
  \boldsymbol{F}_{ z , RF } = - \left [ \frac{ e \Omega E_{0} }{ v_{\smallzero} } \cos \left ( \frac{ \Omega z^{\prime} }{ v_{\smallzero} } + \phi \right ) \right ] z \hat{\operatorname{ \boldsymbol{z} }} \text{.}
\end{equation}
