...
Besides a set of initial conditions, the Analytic Gaussian model presented by Michalik and Sipe (see Section \ref{sec:ms_model}) is only useful for modeling free-space propagation.
...
\begin{equation}
  V(\vec{r} - \vecprime{r}) \rightarrow V(\vec{r} - \vecprime{r}) + \sum_{k} V_{k}^{ext}(\vec{r})
\end{equation}
Following this new term through the derivation, we see that
\begin{equation}
  \begin{split}
    \Phi(\vec{r};t) = & \int d\vecprime{r} \left( V(\vec{r} - \vecprime{r}) + \sum_{k} V_{k}^{ext}(\vec{r}) \right) n(\vecprime{r}; t) \\
    = & \int d\vecprime{r} V(\vec{r} - \vecprime{r}) n(\vecprime{r}; t) + \sum_{k} \int d\vecprime{r} V_{k}^{ext}(\vec{r}) n(\vecprime{r}; t) \\
...
\end{equation}
These external terms are not self-influencing, and thus are not subject to the complexities of the internal Coulombic repulsion force. For clarity we now define $\Phi^{int}(\vec{r};t)$ as the old $\Phi(\vec{r};t)$, which results from the Coulomb force, while our new $K^{force}$ now includes these exteral terms.

This form of $\Phi(\vec{r};t)$ can now be substituted into the definition of $K^{force}$ matrix (\ref{eq:Kforce}), again in the basis defined in \ref{eq:coordinate_vector} 
\begin{equation}
  \begin{split}
  ...
  \end{split}
\end{equation}
Like the potential integrals, we are left with an additional matrix for each external force. Again, for clarity, we now define $K^{int}$ as the $K$ matrix resulting from the internal Coulombic repulsion.

Noting that the spatial derivative of a potential is just the force, we can now define the matrix contributed by each external force is given by
...
Further discussion of magnetic lenses can be found in Section \ref{sec:mag_lens}.
