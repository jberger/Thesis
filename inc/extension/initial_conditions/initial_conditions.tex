...
The initial tranverse properties of the beam are easily understood.
The electron pulse's initial spatial variance is simply
\begin{equation}
\sigma_{\smallT } ( 0 ) = ( \Delta x )^{2} = ( \Delta y)^{2} = \frac{ w^{2} }{ 2 } \punct{.}
\end{equation}
Similarly, a laser pulse of this form has a temporal uncertainty $ \Delta t_{L} = \tau / \sqrt{2} $.
...
In other words, this is the limit where the temperature is low enough to consider the system well described by Fermi-Dirac statistics where the chemical potential is nearly equal to the Fermi energy.
...
%Consequently, we will assume that $ \eta_{ z } $ is a constant over the bunch with a value given by $ \eta_{\smallT } ( 0 ) \cong 3.85 \eta{ z } ( 0 ) $: indeed in this short pulse regime, one can show that over the central $ \sim 75 \% $ of the electrons in the inital photo-generated bunch the variation in $ \sqrt{ \eta_{ z } ( 0 ) } $ is only about $ \pm 25 \% $.
