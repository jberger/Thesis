...
\subsubsection{Constant External Forces}
...
\subsection{Practical Extension to Include Magnetic Lenses and RF Cavities}
...
For the usual case of a spatially compact electron pulse with respect to the dimension of the RF cavity (i.e., $ \Omega z \ll v_{{ \scriptscriptstyle 0}} $ and $ \sqrt{\sigma_{\smallT}} \ll a $), \Eq \eqref{eq:RF_Efield} can be rewritten as
...
\end{equation}
which demonstrates that the RF cavity acts both as a pulse acceleration (first term on RHS of \Eq \eqref{eq:RF_Efield_approx}) and a pulse compression or expansion (second term on RHS of \eqref{eq:RF_Efield_approx}) device.
The second term then gives the desired form (see \Eq \eqref{eq:linear_force}) for the force manipulating the temporal dynamics and characteristics during electron pulse compression;
...
As suggested by the acceleration term in \Eq \eqref{eq:RF_Efield_approx}, this is only a good approximation if the RF field amplitude $E_{0}$ is sufficiently small to not affect the velocity of the peak of the pulse as it propagates through the cavity.
...
\end{equation}
In obtaining \Eq \eqref{eq:RF_Force_r}, which is also of the form for use in \Eq \eqref{eq:linear_force}, we have again used an arbitrary super-Gaussian of order $n$ as a region of influence envelope (see \Eq \eqref{eq:reg_of_influence}) with $L_{RF} \equiv d$, the RF cavity length.
